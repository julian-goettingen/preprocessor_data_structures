...
# this makes a lot of code here unnecessary. refac tor some time? maybe
...
class PreprocessorDataClass():
    ...
    def __init__(self, name, source_string):
        ...
        self.posargs, self.kwargs = util.get_args_from_source_string(source_string)
    ...
    def parse_args(self, arglist):
        ...
        if len(arglist) < len(self.posargs):
        ...
        for line_no, line in enumerate(cleaned_c_source.splitlines()):
            ...
            if m:
                ...
                if mu.group(1) != m.group(1):
                    # TODO: this should not be an exception, but instead be logged and cause exit.
                    # LOGTODO
            ...
            if m:
                raw_args = m.group(1)
                args = util.preprocess_raw_args(raw_args)
...
def extract_valid_kw_arg(s):
    ...
    if len(byeq) != 2:
        raise ValueError(f"To use {s} as keyword-argument it must be of the form argname=value")
