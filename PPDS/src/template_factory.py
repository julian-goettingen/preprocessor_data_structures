...
CONSTRUCTOR_GETTER_REX = re.compile("/\*\s+PPDS_CONSTRUCTORS:\s+(.*?)\*/", re.MULTILINE|re.DOTALL)
...
def get_constructors_from_source_string(source_string):
    ...
    try:
        ...
        raise PPDSParseError("Failed to extract PPDS_CONSTRUCTORS definition from header-file.")

    try:
        raw_constructors = json.loads(json_args)
    except JSONDecodeError as e:
        # todo: highlight problematic character with: \n{e.doc[e.pos]}
        raise PPDSParseError(f"Constructors found in header file, but it is not valid json. Args found: \n{json_args}\n\n Problem: \n{e}\n")

    if not isinstance(raw_constructors, list):
        raise PPDSParseError(f"ARGS must be valid list (json-format), but found {raw_constructors} of type {type}")
    ...
    for r in raw_constructors:
        try:
            ...
            raise PPDSParseError(f"constructor must be valid python regex.\nFound:\n{r}\nProblem:\n{e}")
        else:
            if cmpd.groups != 1:
                raise PPDSParseError(f"constructors are expected to have one group to match the arguments, but found {cmpd.groups} in the following constructor:\n{r}")
            else:
...
def get_args_from_source_string(source_string):
    ...
    if not isinstance(raw_args, dict):
        raise PPDSParseError(f"ARGS must be valid dictionary, but found {raw_args} of type {type}")
