...
// public:
#define {{name}}(i) {{pointer}}[{{name}}_assert_in_bounds(i,#i), {{name}}_WRAP(i)]
...
#define {{name}}_assert_in_bounds(i, expr_name) ((((i) >= {{len}}) || ((i) < -({{len}}))) ? {{name}}_assert_fail(i, expr_name) : ((void)0))

#   else //numpy_wraparound

#define {{name}}_assert_in_bounds(i, expr_name) ((((i) >= {{len}}) || ((i) < 0) )? {{name}}_assert_fail(i, expr_name) : ((void)0))
